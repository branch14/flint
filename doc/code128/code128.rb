...
def code128(code)
   ['moveto', "(^105#{code})", "(includetext height=0.1)", 'code128']
end
...
when :labels
  ...
  paper_offset_y = 12.5574803 # q- 42.519685
  barcode_x = 70
  barcode_y = 20
  text_x = 30
  text_y = 45
...
when :veto
  ...
  (columns * rows).times do |counter|
    ...
    content << [x + barcode_x, y + barcode_y, code128((666000 + counter).to_s)].flatten*' '
